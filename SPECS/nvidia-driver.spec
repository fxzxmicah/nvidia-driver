...
if [ -f %{_sysconfdir}/keys/modsign.key ] && [ -f %{_sysconfdir}/keys/modsign.der ]; then
    ...
    base64 -d << EOF | gunzip > %{_tmpdir}/sign-file
%{sign_tool}
EOF
    chmod +x %{_tmpdir}/sign-file
    for module in /lib/modules/%{kernel_rel}.%{_arch}/kernel/drivers/video/nvidia*.ko; do
        %{_tmpdir}/sign-file sha256 %{_sysconfdir}/keys/modsign.key %{_sysconfdir}/keys/modsign.der $module
    done
    rm -f %{_tmpdir}/sign-file
fi
